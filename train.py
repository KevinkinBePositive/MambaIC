...
def train_one_epoch(
    ...
    return {"train loss":out_criterion["loss"].item(), "train MSE loss": out_criterion["mse_loss"].item(), 
            "train Bpp loss": out_criterion["bpp_loss"].item(), "train PSNR": out_criterion["psnr"], 
            "train MS-SSIM": out_criterion["msssim"].item(), "train Aux loss": aux_loss.item()}
...
def test_epoch(epoch, test_dataloader, model, criterion, type='mse'):
    ...
    return {"test loss":loss.avg, "test MSE loss": mse_loss.avg, 
            "test Bpp loss": bpp_loss.avg, "test PSNR": psnr.avg, 
            "test MS-SSIM": msssim.avg, "test Aux loss": aux_loss.avg}
...
def adaptive_crop(image, target_size=(256, 256)):
    """等比例缩放至最小边≥目标尺寸后裁剪"""
    h, w = image.size
    th, tw = target_size
    if h < th or w < tw:
    ...
    return transforms.functional.center_crop(image, target_size)
...
def main(argv):
    ...
    train_transforms = transforms.Compose([
        transforms.Lambda(lambda x: adaptive_crop(x, args.patch_size)),  # 替换RandomCrop
        transforms.ToTensor()
    ])
    ...
    for epoch in tqdm(range(last_epoch, args.epochs)):
        ...
        writer.add_scalar('train/loss', train_out["train loss"], epoch)
        writer.add_scalar('train/MSE loss', train_out["train MSE loss"], epoch)
        writer.add_scalar('train/Bpp loss', train_out["train Bpp loss"], epoch)
        writer.add_scalar('train/PSNR', train_out["train PSNR"], epoch)
        writer.add_scalar('train/MS-SSIM', train_out["train MS-SSIM"], epoch)
        writer.add_scalar('train/Aux loss', train_out["train Aux loss"], epoch)

        loss = test_epoch(epoch, test_dataloader, net, criterion, type)

        writer.add_scalar('test/loss', loss["test loss"], epoch)
        writer.add_scalar('test/MSE loss', loss["test MSE loss"], epoch)
        writer.add_scalar('test/Bpp loss', loss["test Bpp loss"], epoch)
        writer.add_scalar('test/PSNR', loss["test PSNR"], epoch)
        writer.add_scalar('test/MS-SSIM', loss["test MS-SSIM"], epoch)
        writer.add_scalar('test/Aux loss', loss["test Aux loss"], epoch)

        lr_scheduler.step()

        is_best = loss["test loss"] < best_loss
        best_loss = min(loss["test loss"], best_loss)
